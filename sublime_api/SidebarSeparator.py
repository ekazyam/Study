...
class Listener(EventListener):
    ...
    def on_window_command(self, window, command, args):
        ...
        if(command == 'toggle_tabs' and SidebarSeparator.getCommandHistroyFlag() == False):
...
class SidebarSeparator(sublime_plugin.TextCommand):
    ...
    # This flag indicates that it is executed by the plugin.
    command_execute_flag = False

    @staticmethod
    def getCommandHistroyFlag():
        return SidebarSeparator.command_execute_flag

    @staticmethod
    def setCommandHistoryFlag(flag):
        SidebarSeparator.command_execute_flag = flag
    ...
    def hideTabBar(self, setting_values):
        ...
        if (SidebarSeparator.getShowTabStatus() is None):
            ...
            print('[None-check]', self.getJsonParameter())
        ...
        if (SidebarSeparator.getShowTabStatus() == True):
            ...
            # set command execute flag.
            SidebarSeparator.setCommandHistoryFlag(True)

            # hide tabbar.
            sublime.active_window().run_command('toggle_tabs')

            # set command execute flag.
            SidebarSeparator.setCommandHistoryFlag(False)
